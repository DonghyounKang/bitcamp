...
SQL 문에서 DB 객체(테이블, 뷰, 함수, 트리거)를 생성, 변경, 삭제하는 SQL 명령
...
> create table test1(
  c1 int, 
  c2 float, 
  c3 numeric(6,2),/*소수점 자릿수를 저정하면 부동소수점으로 활용*/
  c4 numeric /*int와 같다*/
  );
  
> insert into test1(c1) values(100);
> insert into test1(c1) values(3.14); /* 소수점 짤림 */
> insert into test1(c2) values(100.98); /*소수점이하 반올림하고 짜름*/
...
/*DBMS중에는 고정 크기인 컬럼의 값을 비교할 때 빈자리까지 검사하는 경우도 있다.
 * 즉, c1='abc'에서는 데이터를 찾지 못하고 c1='abc  '여야만 데이터를 찾는 경우가 있다.
 * 그러나 Mysql은 고정크기 column이더라도 빈자리를 무시하고 데이터를 찾는다.
 * */
