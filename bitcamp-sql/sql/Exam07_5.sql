 ...
 *  1) 두 테이블의 조인 기준이 되는 컬럼 이름이 다를 때 연결될 수 없다.
 *  2) 상관없는 컬럼과 이름이 같을 경우 잘못 연결된다. 
 *  3) 같은 이름의 컬럼이 여러 개 있을 경우 특정 컬럼을 기준으로 연결하지 못한다.
               모든 컬럼의 값이 일치할 경우에만 연결하기 때문이다.. */
...
/*즉 inner join은 기존 컬럼의 값이 일치할 때만 데이터를 연결한다. */
...
/* 강의테이블에서 강의명을 가져오고, 강의실 테이블에서 지점명과 강의실 명을 가져오자 */
select 
l.lno, 
l.titl, 
r.rno, 
 ...
 * 강의실이 아직 지정되지 않은 강의의 경우 강의실 테이블과 연결하지 못해
 * 결과로 출력되지 않는 문제가 있다. */

/*=> 만약 기존의 컬럼의 값과 일치하는 데이터가 없어서 
 ...
 * 만약 lect와 일치하는 데이터가 room에 없더라도 lect데이터를 출력한다.
 */
...
/* 6단계  매니저 이름을 출력
 *  => 매니저 번호는 lect 테이블에 존재
 *  => 매니저 이름은 memb 테이블에 존재
 * */
