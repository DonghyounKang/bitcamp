...
/* 1단계: 과장 또는 대리인 매니저가 담당하고 있는 강의의 수강 신청만 추출하기 */
select 
	la.lano, 
	(select titl from lect where lno=la.lno) as lect_title, 
	(select name from memb where mno=la.mno) as stud_name,
...
where
    ...
    /*lec.manager_no in (select mno from mgr where posi in (200, 301, 302, 304));
     * => 직위 변경시 지속적인 수정이 필요 => 비효율적
     */
    
    
/*
 * 1. SubQuery 이해
 * 2. from절 안에 가상테이블 처럼 SubQuery사용
 * 3. from절 안에 가상테이블 view로 정의하여 사용한다.
 * 4. where절 에도 select문 적용가능(단점: SubQuery의 반복실행 => join보다 효율이 떨어진다)
 *   - 데이터 크기가 작은 경우 SubQuery사용 용이
 * 
 */