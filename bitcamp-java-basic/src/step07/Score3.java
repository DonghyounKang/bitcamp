...
public class Score3 {
    ...
    =>인스턴스의 주소를 파라미터로 받는것이 아니고
          메서드를 호출할 때(연산자를 사용할때) 메서드 앞에 인스턴스의 주소를 적는다.
    =>이 인스턴스의 주소는 내장된 this라는 변수에 복사된다.
    
         그래서 파라미터 대신 this를 사용
    */
     ...
     * 클래스 메서드 
     *  => static이 붙는 메서드
     *  => 특정 인스턴스가 아닌 모든 인스턴스에 대해 작업을 수행할 때 사용하는 메서드
     *  => 사용법
     *    클래스명.메서드명();
     *    레퍼런스.메서드명();
