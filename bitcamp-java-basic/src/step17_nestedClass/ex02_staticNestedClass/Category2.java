...
public class Category2 {
    ...
    // 그렇다면 차라리 각각의 클래스를 멤버 클래스로 별도의 파일로 분리하는 것이 낫지 않을까?
    // => 다음 클래스를 보면 상수를 묶어서 관리하는 용도로 만들었다.
    // => 클래스의 크기도 작다
    // => 이런 클래스를 별도로 파일로 분리하면 여러개의 파일이 생겨서 오히려 관리하기 번거롭다.
    //    즉, 자잘한 클래스들이 여러 파일에 분산되기 때문에 관리하기 어려워진다.
    // => 차라리 이렇게 중첩클래스로 만들면 사용할 때 다음과 같이 계층적으로 작성하기 때문에 이해하기  쉬워진다.
    //     ex) int value = Category.computer.CPU;
    ...
    public static class book {
        public static final int POEM = 100;
