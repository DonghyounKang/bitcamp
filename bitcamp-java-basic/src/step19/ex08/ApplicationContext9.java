// 객체를 저장할 때 클래스 이름으로 저장하고 클래스이름으로 꺼낼 수 있다.
package step19.ex08;
...
public class ApplicationContext9 {
    ...
    private Object createObject(Class clazz) throws Exception{
        try {
            ...
            return clazz.newInstance(); // 기본생성자를 호출하여 객체 생성
        } catch (Exception e) {
    ...
    private Object findObject(Class clazz) throws Exception {
        ...
        if(obj != null) { // 파라미터 타입과 일치하는 객체가 있다면 그 객채를 리턴
            return obj;
        }
        // 만약 objPool에 그런타입의 객체가 없다면 새로 만들어 리턴,보관
    ...
    private boolean containsDefaultType(Constructor constructor) {
        Class [] defaultTypes = {
            ...
            //1) 싱성자 파리미터 정보 꺼내기
            Class[] paramTypes = constructor.getParameterTypes();
            //2) 생성자 파라미터 타입이 primitive타입이거나 Wrapper, String일 경우 이 생성자 호출시
            //   해당 값을 줘서 호출해야 한다.
            //   문제는 -> 어쩐 값을 줘야하는가?
            //   ex) int를 원한다면 int 값으로 얼마를 주어야하는지 결정 불가
            //       그래서 이런 생성자로는 객체 생성불가
            //       이런 생성자인지 검사
            for(Class paramType: paramTypes) {
