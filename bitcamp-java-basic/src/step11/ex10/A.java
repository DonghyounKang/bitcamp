...
public class A {
    ...
    // 파라미터의 타입이나 개수가 다르지만 이름이 같은 메서드를 여러개 만들수 있다.
    ...
    //이유
    //- 메서드르 찾을 때 파라미터 값의 타입으로 찾기 때문이다.
    // 따라서 아래의 메서드는 위의 메서드와 같은 메서드이기 때문에 컴파일 오류
    ...
    */
    ...
    // - 리턴 타입만 다른 메서드를 중복해서 찾을 수 없다.
    //이유?
    // 메서드를 찾을 때 파라미터의 값의 타입으로 찾기 때문이다.
