...
public class Exam03_2 {
    public static void main(String[] args) throws Exception{
        ...
        // 실행하기전 해야할 일 :
        // 1) 
        // test9_4.data를 읽기 전에 Member3 클래스에
        // 전화번호를 저장할 수 있는 인스턴스 변수를 추가하라
        Member3 member = (Member3) in.readObject();
        // Exam02_4의 경우와 달리
        // Member3 객체의 시리얼 데이터를 출력한 다음, 
        // Member3를 변경한 후 데이터를 읽더라도 실행오류 미발생!
        
        //이유?
        // => 클래스 버전 번호가 같기 때문
        //    클래스가 변경도더라도 클래스의 SerialVersionUID 값이 같다면
        //    그래서 readObject()로 호출하여 시리얼 데이터를 읽을 때 오류가 발생하지 않는다.
        
        //serialVersionUID의 사용법
        // => 클래스를 변경하더라도 기존 데이터를 읽는데 문제가 없다면
        //    변경한 클래스의 버전을 이전 버전과 같게 하라
        // => 그러나 클래스의 변경 사항이 너무 많아 이전 데이터를 읽기 부적합하다면
        //    변경한 클래스의 버전을 바꿔 읽을 때 오류가 나게하라
        // => Member3의 경우 이전 버전에셔 tel 변수를 추가하였다.
        //    따라서 이전 데이터를 읽는 데 문제가 없다.
        //    그래서 버전번호 변동 없음
