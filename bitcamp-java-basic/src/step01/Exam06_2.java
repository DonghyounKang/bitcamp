...
public class Exam06_2{
    public static void main(String[] args) {
        ...
        //다음값은 4바이트 메모리 범위를 넘어가는 부동소수점이기 때문에
        //값이 잘린다
        ...
        // why? : IEEE -754 규격에 따라 부동소수점을 2진수로 변환시
        // 정확히 2진수로 딱 떨어지지 않는 경우가 있다.
        // solution : 시스템에서 필요한 만큼 소수점 이하 자리수를 적절히 잘라 사용

        // 결론
        // 32비트(float) 메모리에 부동소수점 저장시 유효자릿수 7자리까지 거의 가능
        // 64비트(double) 메모리에 부동소수점 저장시 유효자릿수 15자리 까지 거의 가능
        // => 32비트 float을 단정도(single-precision)
        //    64비트 double은 두 배 더 정확하게 값을 저장하여 
...
// - 부동소수점을 2(진수로 표현하는 법칙을 정의한 문서가 "IEEE-754"이다.
//
// 3차원 그래픽(부동소수점 연산) => GPU
//
// ### IEEE-754
...
// - 메모리 크기와 비트의 구성
//   => 32비트 float 타입(32-bit single-precision; 단정도) 4바이트
//     [부호비트(1)][지수부(8)][가수부(23)]
//   => 64비트 double 타입(64-bit double-precision; 배정도) 8바이트
//     [부호비트(1)][지수부(11)][가수부(52)]
// - 부호비트(sign bit)
//   음수는 1, 양수는 0.
// - 지수(exponent) Excess-K방식
