...
//   실행을 완료할 수 있게 한다.
//
//[Context Switching] : 실행 환경 교차
// - 동시에 여러 개의 process나 thread를 실행할 때 
//   CPU 사용권한을 뺏어 다른 process나 thread에 주기 전에
//   현재까지 실행한 코드의 위치정보를 저장해야한다.
//   또한 CPU 사용권한을 주기 전에 그 process나 thread가 이전에 어디까지 실행 했었는지
//   이전 실행 위치 정보를 로딩해야한다.
//   즉 실행 위치에 대한 정보를 저장하고 로딩하는 것
